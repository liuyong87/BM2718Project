...
//            PreferencesManager.saveLastAppFlag(Definition.AppFlag.TYPE_MUSIC);// 保存断点记忆应用标识
            PreferencesManager.saveLastMusicUrl(mMusicPlayer.getCurrentPlayingUrl());// 保存断点记忆资源路径
    ...
    private IMusicDataModel.OnRefreshDataListener OnRefreshDataListener = new IMusicDataModel.OnRefreshDataListener() {
        @Override
        public void onUpdateData(List<MusicInfo> dataList) {
...
//                    mMusicPlayer.saveLastMediaInfo();// 保存断点记忆
                    ...
                    mMusicPlayer.clean();
                    stopSelf();
//                    MediaApplication.appExit();  在MediaStorageService 执行此函数
...
//        mRemoteCallbackListMgs.notifyChangePlayStatus(isPlay);   已在start中通知播放状态
    ...
    public void onSwitchFavorite() {
        ...
        mMusicDataModel.onChangeMusicFavoriteState(new IMusicDataModel.OnFavoriteChangeListener() {
            @Override
            public void onChangeFavoriteState(boolean isFavorite) {
                Logutil.i(TAG, "onChangeFavoriteState() isFavorite=" + isFavorite);
                mMusicPlayer.setFavoriteStatusWithCurrentMusic(isFavorite);
                mRemoteCallbackListMgs.notifyChangeFavorite(isFavorite);
            }
        }, mMusicPlayer.getCurrentID3Info());
    }
    ...
    public void onDeleteFromFavorite() {
        mMusicDataModel.onDeleteMusicFavoriteState(new IMusicDataModel.OnFavoriteChangeListener() {
            @Override
            public void onChangeFavoriteState(boolean isFavorite) {
                Logutil.i(TAG, "onChangeFavoriteState() isFavorite=" + isFavorite);
                mMusicPlayer.setFavoriteStatusWithCurrentMusic(isFavorite);
    ...
    private void onRestorePlay() {
        String lastUrl = PreferencesManager.getLastMusicUrl();
    ...
    private void refreshFavoriteState(String url) {
        mMusicDataModel.onLoadFavoriteState(new IMusicDataModel.OnFavoriteChangeListener() {
            @Override
            public void onChangeFavoriteState(boolean isFavorite) {
                mMusicPlayer.setFavoriteStatusWithCurrentMusic(isFavorite);
                mRemoteCallbackListMgs.notifyChangeFavorite(isFavorite);
            }
        }, AppUtil.getUsbFlagFrom(url), url);
    }
