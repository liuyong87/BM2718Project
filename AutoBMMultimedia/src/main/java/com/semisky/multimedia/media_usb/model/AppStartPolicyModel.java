...
public class AppStartPolicyModel implements IAppStartPolicyModel {
    ...
    public void onAutoStartApp(boolean fromScanDoneEvent) {
        ...
        if (((isHightProrityAppRunning && isNaviAtForeground) || (hasInterruptEvent && InterruptEventManager.getInstance().getInterruptEventFromNavi())) && !mIsStartFirstApp){
            Logutil.i(TAG,"onAutoStartApp() "+"background play music");
            startApp(fromScanDoneEvent,true);
            return;
        }else if (isHightProrityAppRunning
                && !isNaviAtForeground
                || hasInterruptEvent
                || mIsStartFirstApp) {
