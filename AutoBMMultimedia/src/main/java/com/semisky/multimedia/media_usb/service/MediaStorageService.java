...
public class MediaStorageService extends Service implements IMediaStorageService {
    ...
    private OnMediaScannerListener mOnMediaScannerListener = new OnMediaScannerListener() {
        ...
        public void onScannerStart(int usbFlag) {
            //如果是关机记录的是收音机，不显示dialog
            if (!SemiskyIVIManager.getInstance().getAllowMusicPlay()) {
                _handler.sendEmptyMessage(MediaScanHandler.MSG_MEDIA_SCAN_STATUS_LOADDING);
            }
            onNotifyMediaScannerStart(usbFlag);
            // 如果有断点记忆资源播放断点记忆
//            // test
//            AppUtil.enterList(Definition.MediaListConst.FRAGMENT_LIST_PHOTO);
        }
        ...
        public void onScanning(int usbFlag) {
            ...
            _handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    mAppStartPolicyModel.onAutoStartApp(false);
                }
            }, 1000);
...
//            unRegisterAutoServiceListener();//扫描完成时不再需要监听打断事件
            onNotifyMediaScannerDone(usbFlag);
            _handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    mAppStartPolicyModel.onAutoStartApp(true);
                    _handler.sendEmptyMessage(MediaScanHandler.MSG_MEDIA_SACAN_DIALOG_DISMISS);
                }
            }, 1000);
//            isScannerComplete = true ;//扫描已经完成
...
//                printCallBackInfo();
    ...
    private static class MediaScanHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MSG_MEDIA_SACAN_DIALOG_DISMISS:
                    mServiceRfr.get().closeMediaScanDialog();
                    break;
                case MSG_MEDIA_SCAN_STATUS_LOADDING:
                    mServiceRfr.get().alertUSBLoadding();
                    break;
                case MSG_MEDIA_SCAN_STATUS_USB_UNMOUNTED:
                    mServiceRfr.get().alertUSBUnmounted();
                    break;
                case MSG_MEDIA_SCAN_STATUS_USB_FAILURE:
                    mServiceRfr.get().alertUSBUFailure(5000);
                    break;
            }
        }
    }
    ...
    protected void handlerMountedUsb(String usbPath) {
        ...
        /**
         * bug 7960 在加载瞬间退出泊车，依然启动多媒体，执行到启动APP时，没有高优先级应用在前台，
         * 故此启动，接收到mount事件后，如前台有高优先级应用时便持影响，更改如下
         */
        SemiskyIVIManager.getInstance().isHighPriorityAppRunning();
        registerAutoServiceListener();//关于中间件的所有打断监听事件
        //设置一个状态到中间件，（上下一曲问题，主要为应用启动瞬间使用mode上下一曲带来的问题）
//        SemiskyIVIManager.getInstance().setCurrentAppStatus(AutoConstants.AppType.MEDIA_MUSIC, AutoConstants.CurrentAppStatus.STATE_PREPARE);
    ...
    protected void handlerUnmountedUsb(String usbPath) {
        ...
        _handler.sendEmptyMessage(MediaScanHandler.MSG_MEDIA_SACAN_DIALOG_DISMISS);
        VideoAudioFocusModel.getInstance(this).onAbandonAudioFocus();// 注册视频音频焦点
        ...
        //设置 多媒体不需要再使用当前状态
//        SemiskyIVIManager.getInstance().setCurrentAppStatus(AutoConstants.AppType.MEDIA_MUSIC, AutoConstants.CurrentAppStatus.STATE_INVALID);
        MediaApplication.appExit();
        stopSelf();
    }
